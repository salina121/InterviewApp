...
namespace InterviewBot.Services
{
    ...
    public class OpenAIAgentService : IAIAgentService
    {
        ...
        private readonly string idioma = CultureInfo.CurrentCulture.Name;
        ...
        public async Task<string> AskQuestionAsync(string message)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    ...
                    if (idioma == "es")
                    {
                        setLanguage = "The questions to be in spanish";
                    }
                    else {
                        setLanguage = "The questions to be in english";
                    }
                        var requestBody = new
                        {
                            model = _config.Model,
                            ...
                            {
                                role = "system",
                                content = "You are an expert technical interviewer. Provide clear, concise, and helpful responses. When generating questions, format them as a numbered list. When evaluating interviews, provide a score out of 100 and detailed feedback. " +  setLanguage
                            },
                            new
                            {
                                role = "user",
